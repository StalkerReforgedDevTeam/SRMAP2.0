modded class SCR_InventoryUIInfo : UIInfo
{
    ...
    string PropertiesBloodLevel = "#armst_uiunfoitem_Blood"; 
    string PropertiesStaminaLevel = "#armst_uiunfoitem_Stamina"; 
    string PropertiesWaterLevel = "#armst_uiunfoitem_Water"; 
    string PropertiesEatLevel = "#armst_uiunfoitem_Eat"; 
	
    string PropertiesConditionLevel = "#armst_uiunfoCondition";
	...
	override string GetInventoryItemDescription(InventoryItemComponent item)
	{
		...
		bool hasDescription = (format && !format.IsEmpty());
		
		// Start with minimal or no spacing
		string formattedText = "";
		
		// Only add spacing and description if it exists
		if (hasDescription)
		{
			formattedText = "                                                                                    ";
        ...
        bool isArtefact = (artStats != null);
        
        if (artStats)
        {
            // Check if any artefact properties exist before adding header
            bool hasArtefactProps = (
                artStats.m_fPropertiesToxicLevel != 0 ||
                artStats.m_fPropertiesRadiactiveLevel != 0 ||
                artStats.m_fPropertiesPsyLevel != 0 ||
                artStats.m_fPropertiesPhysicalsLevel != 0 ||
                artStats.m_fPropertiesThermoLevel != 0 ||
                artStats.m_fPropertiesElectroLevel != 0 ||
                artStats.m_fPropertiesHealthLevel != 0 ||
                artStats.m_fPropertiesBloodLevel != 0 ||
                artStats.m_fPropertiesStaminaLevel != 0 ||
                artStats.m_fPropertiesWaterLevel != 0 ||
                artStats.m_fPropertiesEatLevel != 0
            );
		...
		{	
		    ...
		    float ConditionLevel = playerStats.GetConditionLevel();
		    
		    // Check if this is a wearable item (has protection stats)
		    bool isWearable = (physicalsLevel > 0 || radiactiveLevel > 0 || toxicLevel > 0 || psyLevel > 0 || thermoLevel > 0 || electroLevel > 0);
		    
		    // NO PRICE DISPLAYED FOR ANY ITEMS
		    ...
		    if (isWearable)
			{
				...
				// ONLY SHOW CONDITION FOR CLOTHING (items with protection stats)
				formattedText += string.Format("\n\n%2: %1", (int)ConditionLevel, PropertiesConditionLevel);
			}
			
	        ARMST_ItemUseComponent consumable = ARMST_ItemUseComponent.Cast(item.GetOwner().FindComponent(ARMST_ItemUseComponent));
	        if (consumable)
			{
				// SHOW EFFECTS FOR: WEARABLES OR ARTIFACTS
				// HIDE EFFECTS FOR: CONSUMABLES (food, medicine, etc.)
				if (!isWearable && !isArtefact)
				{
					// Skip effects display for regular consumables (not artifacts or wearables)
				...
				// Check if any consumable effects exist
				bool hasEffects = (HP != 0 || Eat != 0 || Water != 0 || Radiactive != 0 || Toxic != 0 || Psy != 0);
